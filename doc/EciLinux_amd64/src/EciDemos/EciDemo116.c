...
ECI_RESULT EciCanDemo116  ( DWORD   dwHwIndex,
                            DWORD   dwCtrlIndex);
ECI_RESULT EciLinDemo116  ( DWORD   dwHwIndex,
                            DWORD   dwCtrlIndex);
...
ECI_RESULT EciDemo116(void)
{
  ...
  hResult = ECI116_Initialize(_countof(astcHwPara), astcHwPara);
  ECIDEMO_CHECKERROR(ECI116_Initialize);

  //*** Retrieve hardware info
  if(ECI_OK == hResult)
  {
    //*** Retrieve hardware info
    hResult = ECI116_GetInfo(dwHwIndex, &stcHwInfo);
    ECIDEMO_CHECKERROR(ECI116_GetInfo);
    if(ECI_OK == hResult)
  ...
  if(ECI_OK == hResult)
  {
    ...
    {
      //*** Start CAN Demo
      hResult =  EciCanDemo116(dwHwIndex, dwCtrlIndex);
      ECIDEMO_CHECKERROR(EciCanDemo116);
    }
    else
    {
  ...
  if(ECI_OK == hResult)
  {
    ...
    {
      //*** Start LIN Demo
      hResult =  EciLinDemo116(dwHwIndex, dwCtrlIndex);
      ECIDEMO_CHECKERROR(EciLinDemo116);
    }
    else
    {
      //*** Ignore if no controller was found
  ...
  OS_Printf("-> Returning from ECI Demo for USB-to-CAN FD <-\n");
                         ...
                         DWORD dwCtrlIndex)
{
  ...
  if(ECI_OK == hResult)
  {
    ...
    hResult = ECI116_CtrlOpen(&dwCtrlHandle, dwHwIndex, dwCtrlIndex, &stcCtrlConfig);
    ECIDEMO_CHECKERROR(ECI116_CtrlOpen);
  }
  ...
  if(ECI_OK == hResult)
  {
    ...
    hResult = ECI116_CtrlGetCapabilities(dwCtrlHandle, &stcCtrlCaps);
    ECIDEMO_CHECKERROR(ECI116_CtrlGetCapabilities);
    if(ECI_OK == hResult)
    {
  ...
  if(ECI_OK == hResult)
  {
    hResult = ECI116_CtrlStart(dwCtrlHandle);
    ECIDEMO_CHECKERROR(ECI116_CtrlStart);
  }
  ...
  if(ECI_OK == hResult)
  {
    ...
    for(dwIndex=0; dwIndex < dwTxMsgCount; dwIndex++)
    {
      ...
      #ifdef ECIDEMO_HWUSEPOLLINGMODE
      {
        ...
        do
        {
          hResult = ECI116_CtrlSend( dwCtrlHandle, &stcCtrlMsg, 0);
          if(ECI_OK != hResult)
            { OS_Sleep(1);}
        }while((ECI_OK != hResult) && ((OS_GetTimeInMs() - dwStartTime) < ECIDEMO_TX_TIMEOUT));
      }
      #else
        hResult = ECI116_CtrlSend( dwCtrlHandle, &stcCtrlMsg, ECIDEMO_TX_TIMEOUT);
      #endif //ECIDEMO_HWUSEPOLLINGMODE
      if(ECI_OK != hResult)
      {
        OS_Printf("Error while sending CAN Messages\n");
        ECIDEMO_CHECKERROR(ECI116_CtrlSend);
        hResult = ECI_OK;
        break;
      }
      else
      {
        ...
        hResult = ECI116_CtrlReceive( dwCtrlHandle, &dwCount, astcCtrlMsg, 0);
  ...
  if(ECI_OK == hResult)
  {
    ...
    while(dwRxTimeout >= (dwCurrentTime - dwStartTime))
    {
      ...
      #ifdef ECIDEMO_HWUSEPOLLINGMODE
      {
        ...
        {
          dwCount = 1;
          hResult = ECI116_CtrlReceive( dwCtrlHandle, &dwCount, &stcCtrlMsg, 0);
      ...
      {
        dwCount = 1;
        hResult = ECI116_CtrlReceive( dwCtrlHandle, &dwCount, &stcCtrlMsg, ECIDEMO_RX_TIMEOUT);
      }
      #endif //ECIDEMO_HWUSEPOLLINGMODE
  ...
  if(ECI_OK == hResult)
  {
    hResult = ECI116_CtrlStop(dwCtrlHandle, ECI_STOP_FLAG_NONE);
    ECIDEMO_CHECKERROR(ECI116_CtrlStop);
  }
  ...
  if(ECI_OK == hResult)
  {
    hResult = ECI116_CtrlStop(dwCtrlHandle, ECI_STOP_FLAG_RESET_CTRL);
    ECIDEMO_CHECKERROR(ECI116_CtrlStop);
  }

  //*** Close ECI Controller
  ECI116_CtrlClose(dwCtrlHandle);
